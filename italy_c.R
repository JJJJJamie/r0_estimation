...
deaths<- Predictive_death$Italy
deaths[50:51] <- c(189,250) # data correction
deaths <- deaths[which(deaths>10)] # only keeping daily new deaths > 10 # 2nd March
t_deaths <- length(deaths)
t_before_fd <- 30 # 1st feb
t_total <- t_deaths + t_before_fd

bond_n <- 6 # smooth out the death data for better estimation
